...
class CardControllerWidget extends StatelessWidget {
  ...
  CardControllerWidget(
      ...
      required this.align,
      this.textStyle})
      : cardMaxHeight = cardViewPagerHeight * (2 / 3),
        cardMaxWidth = cardViewPagerWidth! * (5 / 6);
  ...
  double getTop(double cardHeight, double viewHeight, int i) {
    ...
    if (diffAbs > 0.0 && diffAbs <= 1.0) {
      ...
      if (diff >= 0) {
        return basePosition - (cardMaxHeight * (6 / 9)) - cardMaxHeight * (2 / 9) * (diffAbs - diffAbs.floor()).abs();
      } else {
        return basePosition + (cardMaxHeight * (6 / 9)) + cardMaxHeight * (2 / 9) * (diffAbs - diffAbs.floor()).abs();
      }
    } else {
